# 초급자를 위한 '장고 베이스캠프 강의'의 final 프로젝트입니다.
    * 이 강의는 유튜브에 공개하는 강의입니다.
    * 이 강의는 초급자 대상 강의입니다. 
    * 중급자 분들은 AskDajgno 이진석님의 유튜브 클론 강의를 시청해주세요. 유튜브에 있습니다.

    * 개발하는 방법은 여러가지가 있습니다.
        * 초급자라면 FBV로 개발해보시길 권해드립니다.
        * 중급자라면 동일 프로젝트를 CBV로 개발해보시고 DRF의 FBV, CBV로 개발해보시길 권해드립니다.
    * 개발 스택은 Django + Tailwind입니다.
    * 여기 타이핑되어있는 명령어와 코드는 '장고 베이스캠프' 강의에서만 공개됩니다.
    * UI는 메인화면과 상세 페이지만 간단하게 작업합니다.
        ...
        <nav>
            <ul class="flex justify-end gap-2">
                {% if user.is_authenticated %}
                    ...
                    <li class="inline-block ml-3 border border-gray-300 rounded px-2 py-[5px] text-xs"><a href="{% url 'user_logout' %}">로그아웃</a></li>
                {% else %}
